		...
		struct sec_battery_info *battery)
{
	...
	ret = of_property_read_string(np,
		"battery,regulator_name", (char const **)&pdata->regulator_name);
	if (ret)
		pr_info("%s: Voltage regulator name is Empty\n", __func__);	
	...
	if (pdata->chg_temp_check_type) {
		...
		ret = of_property_read_u32(np, "battery,tx_high_temp_check",
					   &pdata->tx_high_temp_check);
		if (ret)
			pr_info("%s : tx_high_temp_check is Empty\n", __func__);

		if(pdata->tx_high_temp_check) {
			ret = of_property_read_u32(np, "battery,tx_high_temp",
						   &pdata->tx_high_temp);
			if (ret)
				pr_info("%s : tx_high_temp is Empty\n", __func__);

			ret = of_property_read_u32(np, "battery,tx_high_temp_recovery",
						   &pdata->tx_high_temp_recovery);
			if (ret)
				pr_info("%s : tx_high_temp_recovery is Empty\n", __func__);

			ret = of_property_read_u32(np, "battery,charging_limit_current_with_tx",
						   &pdata->charging_limit_current_with_tx);
			if (ret)
				pr_info("%s : charging_limit_current_with_tx is Empty\n", __func__);			
		}
	}
