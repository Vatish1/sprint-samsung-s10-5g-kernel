...
static void s2asl01_set_supllement_mode(struct s2asl01_switching_data *switching, bool onoff)
{
	...
	//if (!switching->in_ok) {
	//	pr_info("%s: INOK = %d, malfunction\n", __func__, switching->in_ok);
	//	return ;
	//}
...
static int s2asl01_switching_parse_dt(struct device *dev, struct s2asl01_platform_data *pdata)
{
	...
	if (np == NULL) {
		...
		if(pdata->bat_type & LIMITER_MAIN) {
			...
			ret = pdata->bat_int = of_get_named_gpio_flags(np, "limiter,main_bat_int",
					0, &irq_gpio_flags);
			if (ret < 0)
				dev_err(dev, "%s : can't get main_bat_int\r\n", __FUNCTION__);
			...
			ret = pdata->bat_int = of_get_named_gpio_flags(np, "limiter,sub_bat_int",
					0, &irq_gpio_flags);
			if (ret < 0)
				dev_err(dev, "%s : can't get sub_bat_int\r\n", __FUNCTION__);
				...
				const struct i2c_device_id *id)
{
	...
	wake_lock_init(&switching->limiter_wake_lock, WAKE_LOCK_SUSPEND,
			"limiter_wakelock");
	...
	pr_info("%s [%s]: int = %d, en = %d\n",
		__func__,
		current_limiter_type_str[switching->pdata->bat_type],
		gpio_get_value(switching->pdata->bat_int),
...
err_irq:
err_pdata_free:
	wake_lock_destroy(&switching->limiter_wake_lock);
