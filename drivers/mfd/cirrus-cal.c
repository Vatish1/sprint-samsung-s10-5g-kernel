...
#if defined(CONFIG_SEC_WINNERLTE_PROJECT) || defined(CONFIG_SEC_WINNERX_PROJECT)
#ifdef CONFIG_SEC_FACTORY
	struct snd_soc_codec *codec_right;
	struct snd_soc_codec *codec_left;
#endif
#endif
...
int cirrus_cal_codec_add(struct snd_soc_codec *codec, bool right_channel_amp)
{
	if (cirrus_cal) {
		if (right_channel_amp)
			cirrus_cal->codec_right = codec;
		else
			cirrus_cal->codec_left = codec;
		dev_info(cirrus_cal->dev, "linked codec %s\n",
			 right_channel_amp ? "right" : "left");
	}
					...
					const char *buf, size_t size)
{
	...
	if (ret == 0 && reinit == 1) {
		if (cirrus_cal->codec_left)
			cs35l41_reinit(cirrus_cal->codec_left);

		if (cirrus_cal->codec_right)
			cs35l41_reinit(cirrus_cal->codec_right);
	}
