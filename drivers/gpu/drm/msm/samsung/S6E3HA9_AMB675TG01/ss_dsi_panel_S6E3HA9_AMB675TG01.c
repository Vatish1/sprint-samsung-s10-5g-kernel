...
*/
#include "ss_dsi_panel_S6E3HA9_AMB675TG01.h"
#include "ss_dsi_mdnie_S6E3HA9_AMB675TG01.h"
#include "ss_dsi_interpolation_S6E3HA9_AMB675TG01.h"
...
static struct dsi_panel_cmd_set *ss_hbm_etc(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	*/
	...
	hbm_etc_cmds->cmds[3].msg.tx_buf[1] = vdd->br.elvss_value2;
	...
	if (vdd->gradual_acl_val) {
		...
		hbm_etc_cmds->cmds[5].msg.tx_buf[6] = 0;	/* 0% */
...
static int ss_elvss_read(struct samsung_display_driver_data *vdd)
{
	...
	vdd->br.elvss_value1 = elvss_b5[0]; /*0xB5 57th OTP value*/
	vdd->br.elvss_value2 = elvss_b5[1]; /*0xB5 58th OTP value*/
...
static struct dsi_panel_cmd_set *ss_elvss(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	/* 	0xB5 1th TSET
		...
		BIT(6) ~ BIT(0) is real temperature.
	*/
	elvss_cmds->cmds[0].msg.tx_buf[1] = vdd->temperature > 0 ?
			vdd->temperature : (char)(BIT(7) | (-1*vdd->temperature));
	...
	elvss_57th_val = vdd->br.elvss_value1;
		...
		struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	vdd->br.aor_data = (hmt_aid_cmds->cmds->msg.tx_buf[1] << 8)
...
static void samsung_panel_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->panel_func.samsung_smart_get_conf = smart_get_conf_S6E3HA9_AMB675TG01;

	/* GAMMA FLASH */
	vdd->panel_func.gen_hbm_interpolation_gamma = gen_hbm_interpolation_gamma_S6E3HA9_AMB675TG01;
	vdd->panel_func.gen_hbm_interpolation_irc = gen_hbm_interpolation_irc_S6E3HA9_AMB675TG01;
	vdd->panel_func.gen_normal_interpolation_irc = gen_normal_interpolation_irc_S6E3HA9_AMB675TG01;
	vdd->panel_func.samsung_flash_gamma_support = flash_gamma_support_S6E3HA9_AMB675TG01;
	vdd->panel_func.samsung_interpolation_init = init_interpolation_S6E3HA9_AMB675TG01;
	...
	vdd->panel_func.samsung_smart_get_conf_hmt = smart_get_conf_S6E3HA9_AMB675TG01_hmt;
...
//	vdd->debug_data->panic_on_pptimeout = true;
	...
	/* Set IRC init value */
	vdd->br.irc_mode = IRC_FLAT_GAMMA_MODE;
...
static int __init samsung_panel_initialize(void)
{
	...
	char panel_string[] = "ss_dsi_panel_S6E3HA9_AMB675TG01_WQHD";
