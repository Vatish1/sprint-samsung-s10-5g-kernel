...
#endif
{
	...
	if (unlikely(vdd->panel_dead)) {
		pr_info("recover panel, set vdd(%d)->panel_dead to false \n", vdd->ndx);
...
static int dsi_panel_parse_panel_mode(struct dsi_panel *panel)
{
	...
	if (!mode) {
		pr_err("[%s] Fallback to default panel mode\n", panel->name);
...
static int dsi_panel_parse_gpios(struct dsi_panel *panel)
{
	...
	pr_err("%s: reset_gpio(%d)\n", __func__, panel->reset_config.reset_gpio);
error:
	return rc;
}
				...
				int topology_override)
{
	...
	if (rc)
		pr_err("failed to get dms info, rc=%d\n", rc);

	rc = dsi_panel_parse_esd_config(panel);
	if (rc)
		pr_err("failed to parse esd config, rc=%d\n", rc);
	else {
		u8 *esd_mode = NULL;
			...
			int topology_override)
{
	...
	pr_info("%s, new dsi_display_mode_priv_info at %pK, CPU%d\n",
...
#if defined(CONFIG_DISPLAY_SAMSUNG)
       if (!ss_panel_attach_get(panel->panel_private)) {
               pr_info("PBA booting, skip to disable panel\n");
               goto skip_cmd_tx;
       }
#endif
