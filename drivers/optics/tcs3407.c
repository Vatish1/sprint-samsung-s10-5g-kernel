...
static int ams_smux_set(ams_deviceCtx_t *ctx)
{
	...
	if ((ctx->deviceId == AMS_TCS3408) || (ctx->deviceId == AMS_TCS3408_UNTRIM)) {
		ret = ams_setByte(ctx->portHndl, DEVREG_ENABLE, 0x01); //PON

		/* Send smux remap sequence */
		// Testing only
		ret = ams_setBuf(ctx->portHndl, DEVREG_RAM_START, &smux_tcs3408_data[0], sizeof(smux_tcs3408_data));
		/* execute the smux command */
		AMS_WRITE_S_MUX();//SMUX Write from RAM to chain
		ret = ams_setByte(ctx->portHndl, DEVREG_ENABLE, 0x11);//PON + SMUXEN
