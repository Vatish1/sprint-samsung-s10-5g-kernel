...
static int get_smd_item_result(char *buf, int piece)
{
	struct param_quest_t param_quest_data;
	int iCnt;
			     ...
			     struct device_attribute *attr, char *buf)
{
	...
	// 2.1. in case of SMD, let's save result to param
				      ...
				      struct device_attribute *attr, char *buf)
{
	int ret = 0;
	int i = 0;
	struct param_quest_ddr_test_result_t param_quest_ddr_test_result_data;

	if (!sec_get_param
	    (param_index_quest_ddr_result, &param_quest_ddr_test_result_data)) {
		QUEST_PRINT("%s : fail - get param!! param_quest_ddr_test_result_data\n",
		       __func__);
		goto err_out;
	}
	...
	ret =
	    snprintf(buf, BUFF_SZ, "Total : %d\n\n",
		    param_quest_ddr_test_result_data.ddr_err_addr_total);
	for (i = 0; i < param_quest_ddr_test_result_data.ddr_err_addr_total; i++) {
		ret +=
		    snprintf(buf + ret - 1, BUFF_SZ, "[%d] 0x%llx\n", i,
			    param_quest_ddr_test_result_data.ddr_err_addr[i]);
	}
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	for (iCnt = 0; iCnt < ITEM_CNT; iCnt++) {
		...
		if (!get_smd_item_result(strResult, iCnt))
			goto err_out;
